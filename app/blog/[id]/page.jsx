...
export default async function BlogDetailPage({ params }) {
  ...
  const response = await fetch(`https://jsonplaceholder.org/posts/${id}`, {
    next: { revalidate: 60 }, // Enable caching with revalidation
  });

  if (!response.ok) {
    return notFound(); // If the post is not found, return a 404 page
  }
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row justify-around w-full  md:max-w-[1800px] md:ml-[69px]  md:mr-[45px] md:mt-36 mx-auto">
       ...
       <p className="bg-[#C9C5C580] w-[137px] h-[46px] flex justify-center items-center mb-2  rounded-[10px] text-[24px] px-[11px] py-[10px] ">{blog.category}</p>
       </div> 
